...
contextBridge.exposeInMainWorld('nativeSqlite', {
  ...
  importLocalSnapshotSync: (uid, snapshot) => ipcRenderer.sendSync('sqlite:import-local-snapshot-sync', uid, snapshot),
  putCollection: (uid, col, records) => ipcRenderer.invoke('sqlite:put-collection', uid, col, records),
  putCollectionSync: (uid, col, records) => ipcRenderer.sendSync('sqlite:put-collection-sync', uid, col, records),
